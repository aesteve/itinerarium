...
pub trait Handler: Send + Debug + Sync + DynClone {
    ...
    fn handle_res(&mut self, res: &mut Response<Body>) -> HandlerResponse;
}
...
mod tests {
    ...
    async fn test_mut_handler() {
        #[derive(Debug, Clone)] struct CountHandler { counter: u32 }
        impl Handler for CountHandler {
            ...
            fn handle_res(&mut self, res: &mut Response<Body>) -> HandlerResponse {
                self.counter += 1;
                res.headers_mut().insert("X-Count", HeaderValue::from(self.counter));
                Continue
            }
        }
        let gw_port = 7300;
        ...
        tokio::spawn(async move {
            let mut api = Api::http("127.0.0.1", backend_port, path.to_string()).unwrap();
            api.register_handler(Box::new(CountHandler { counter: 0 }));
            start_local_gateway(gw_port, vec![api]).await
        });
        wait_for_gateway(gw_port).await;
        let client = Client::new();
